...
- **MongoDB**
- **Backend** (Express, порт `8787`)
- **Frontend** (Nginx, порт `8080`, проксирует `/api` и `/ws` на backend)
...
```bash
# (опционально) поправьте .env
# JWT_SECRET настоятельно рекомендуется заменить на длинную случайную строку
...
Дефолтный админ (создаётся автоматически при старте backend):
...
> Для dev-режима настройки лежат в `backend/.env` и `frontend/.env` (есть `.env.example`).

---

## Архитектура (очень коротко)

- Backend держит **in-memory store** со свечами/тикерами и считает индикаторы.
- График подписывается на `/ws/chart` (kline stream по символу+интервалу).
- Сканеры и боты получают обновления по WebSocket каналам:
  - `/ws/scanners?token=...`
  - `/ws/bots?token=...`
...
### 1) Chart
`ws://<backend>/ws/chart?symbol=BTCUSDT&interval=1m`

Без авторизации (публичные данные).

### 2) Scanners (требуется token)
`ws://<backend>/ws/scanners?token=<JWT>`

При подключении приходит `scannersSnapshot`.

### 3) Bots (требуется token)
`ws://<backend>/ws/bots?token=<JWT>`

При подключении приходит `botsSnapshot`.
...
# Настройки backend (.env) и на что они влияют

Все переменные читаются из `backend/src/util/config.js`. В Docker чаще используется корневой `.env`.

## Сервер и авторизация
- `PORT` (деф. `8787`) — порт backend
- `CORS_ORIGIN` (деф. `http://localhost:5173`) — разрешённый origin для браузера
- `JWT_SECRET` (деф. `dev-jwt-secret-change-me`) — подпись JWT (обязательно поменять)
- `JWT_TTL` (деф. `7d`) — срок жизни токена
- `ADMIN_EMAIL`, `ADMIN_PASSWORD` — bootstrap админ (создаётся при старте)

## MongoDB
- `MONGO_URI` (деф. `mongodb://localhost:27017`)
- `MONGO_DB` (деф. `binance_futures_analytics`)

## Шифрование секретов в БД (опционально)
- `DATA_ENCRYPTION_KEY` — если задан, Binance/OpenAI ключи **шифруются при хранении** (AES-256-GCM).
  - ⚠️ Не меняйте ключ после записи данных, иначе старые значения не расшифруются.

## Кэш свечей / нагрузка
- `MAX_BARS` (деф. `600`) — сколько баров хранить на (symbol,timeframe) в памяти
- `DEFAULT_UNIVERSE_LIMIT` (деф. `80`) — топ-N по объёму для сканеров по умолчанию
- `FETCH_CONCURRENCY` (деф. `6`) — параллельность подкачки клинов REST

## Rolling REST cache / pacing (важно против rate-limit)
- `CACHE_TICK_MS` (деф. `1000`)
- `UNIVERSE_REFRESH_MS` (деф. `10000`)
- `REST_KEY_MIN_GAP_MS` (деф. `250`)
- `REST_GLOBAL_MIN_GAP_MS` (деф. `90`)
- `REST_BASE_BACKOFF_MS` (деф. `500`)
- `REST_MAX_BACKOFF_MS` (деф. `20000`)
- `REST_MAX_RETRIES` (деф. `2`)

## Signed/private throttling (LIVE/DEMO торговля)
- `SIGNED_MIN_INTERVAL_MS` (деф. `350`)
- `SIGNED_MAX_CONCURRENT` (деф. `1`)
- `SIGNED_ACCOUNT_SNAPSHOT_TTL_MS` (деф. `10000`)

## CryptoPanic (опционально)
- `CRYPTOPANIC_TOKEN` — включает загрузку новостей
- `CRYPTOPANIC_PUBLIC` (деф. `true`)
- `CRYPTOPANIC_CACHE_TTL_MS` (деф. `60000`)

## OpenAI (опционально, для “tune bot”)
- `OPENAI_API_KEY` (для миграции/совместимости, в multi-user ключ хранится per-user)
- `OPENAI_MODEL` (деф. `gpt-4.1-mini`)
- `PERSIST_OPENAI_KEYS`, `OPENAI_KEYS_FILE` — опционально

---

# Мини-справочник API

> В UI всё уже используется, но иногда удобно дергать вручную.
...
- `GET /api/scanners`
- `POST /api/scanners`
- `POST /api/scanners/:id/start`
- `POST /api/scanners/:id/stop`
...
- `POST /api/bots/:id/backtest/start`
- `POST /api/bots/:id/backtest/stop`
...
## Примечание про “TradingView”

Используется `lightweight-charts` (open-source) для TradingView-подобного UI.
Это **не** проприетарная TradingView Charting Library (для неё нужна отдельная лицензия).
